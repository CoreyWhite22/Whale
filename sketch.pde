int numLayers = 65; //increase to improve quality (toggle speed and frameRate too)
Layer[] layers = new Layer[numLayers];

float size = 25;
float speed = .35; //50 layers = .03 speed = 30 frameRate,  30/.05/20,  20/.12/15,  10/.2/5
float bx, by;
...
// Increase the denominator to reduce the slowdown effect. For example, change 100 to 200.
float adjustedSpeed = speed + (1 - min(distance / 2300, 1)) * 0.5; // Adjust the denominator to control the slowdown effect

  if (first == 1) {
    X = (adjustedSpeed/numLayers*x2+X)/(adjustedSpeed/numLayers+1);
    Y = (adjustedSpeed/numLayers*y2+Y)/(adjustedSpeed/numLayers+1);
  } else {
    X = BX;
    Y = BY;
  }
